...
//	    std::cout<<"PUNTOS PARA TRABAJAR EL CENTROID: "<<v0<<std::endl<<v1<<std::endl<<v2<<
//		    std::endl<<v3<<std::endl<<v4<<std::endl<<v5<<std::endl<<v6<<std::endl<<v7<<std::endl;
...
long double p = 0.l;
long double s = 1.l-p;
Cell_Centroids[m_id].x = (s*centerS.x + p*centerP.x);
Cell_Centroids[m_id].y = (s*centerS.y + p*centerP.y);
Cell_Centroids[m_id].z = (s*centerS.z + p*centerP.z);

//Cell_Centroids[m_id].x = (p0.x + p1.x + p2.x + p3.x + p4.x + p5.x + p6.x + p7.x + p8.x + p9.x + p10.x + p11.x)/(12.0f);
//Cell_Centroids[m_id].y = (p0.y + p1.y + p2.y + p3.y + p4.y + p5.y + p6.y + p7.y + p8.y + p9.y + p10.y + p11.y)/(12.0f);
//Cell_Centroids[m_id].z = (p0.z + p1.z + p2.z + p3.z + p4.z + p5.z + p6.z + p7.z + p8.z + p9.z + p10.z + p11.z)/(12.0f);

//Cell_Centroids[m_id] = Shell_centroid;
...
//            Cell_Centroids[5] = cS;
//            Cell_Centroids[6] = cN;
//            Cell_Centroids[7] = Shell_centroid;
//            Cell_Centroids[8] = Shell_centroid;
//            Cell_Centroids[9] = Shell_centroid;
//            Cell_Centroids[10] = Shell_centroid;
//            Cell_Centroids[11] = Shell_centroid;
//            Cell_Centroids[12] = Shell_centroid;

}

void structuredHexMesh::SweepFace(const vec3 MeshSurface[], const int& c_Xwide, const int& c_Zheight, const int& c_Ysweep, const long double& sweepValue, const vec3& origin)
    {
    if(Mesh==0)
    {
        m_H = c_Zheight;
